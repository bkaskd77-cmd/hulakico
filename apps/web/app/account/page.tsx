...
export default async function AccountPage({
  ...
  searchParams: Promise<{ ops?: string; page?: string; copy?: string }>;
}) {
  const token = await readSessionToken();
    ...
    <div className="shell-sky min-h-dvh px-6 py-16 sm:px-10">
      <div className="mx-auto max-w-lg space-y-6">
        <div className="rounded-lg border border-[color-mix(in_srgb,var(--off-white)_14%,transparent)] bg-[var(--navy-elevated)] p-8">
          ...
          {opsDenied ? (
            <p className="mt-3 text-sm text-[var(--danger)]">Ops tower is limited to OPS/ADMIN accounts.</p>
          ) : null}
          {copyError ? <p className="mt-3 text-sm text-[var(--danger)]">{decodeURIComponent(copyError)}</p> : null}
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link href="/book" className="rounded-md bg-[var(--gold)] px-4 py-2 text-sm font-semibold text-[var(--navy)]">Book a shipment</Link>
            <Link href="/ops" className="rounded-md bg-[var(--teal)] px-4 py-2 text-sm font-semibold text-[var(--off-white)]">Ops tower</Link>
            <form action={signOutAction}>
              <button type="submit" className="rounded-md border border-[var(--muted)] px-4 py-2 text-sm text-[var(--off-white)]">Sign out</button>
            </form>
          </div>
          ...
          {list.total === 0 ? (
            ...
            <>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <span className="text-xs text-[var(--muted)]">Page {list.page} of {list.pageCount} · {list.total} total</span>
                {pageNumbers.map((n) => (
                  <Link key={n} href={n === 1 ? "/account" : `/account?page=${n}`}
                    className={`rounded px-2 py-1 text-xs font-semibold ${
                      n === list.page ? "bg-[var(--gold)] text-[var(--navy)]" : "border border-[var(--teal)] text-[var(--teal)]"
                    }`}>{n}</Link>
                ))}
                {list.page < list.pageCount ? (
                  <Link href={`/account?page=${list.page + 1}`} className="text-xs font-semibold text-[var(--gold)] underline">View more</Link>
                ) : null}
              </div>
            </>
          )}
