...
**Status:** Step 11 complete (awaiting approval). Phase 1 checklist finished.
...
| 10 | Python ETA risk + document QC wired | **DONE** |
| 11 | COD basics + landing polish + harden | **DONE** (awaiting approval) |
...
Human: book a domestic COD shipment → confirm ledger row on `/ops/cod` → mark collected → check polished `/` landing → approve → optionally commit.  
Phase 1 complete. Next: Phase 2 (advanced booking UX, ops role gates, customer exception replies) when ready.
